...
def refine_corner(img, sess, x,y_eval, retainFactor):
    ans_x = 0.0
    ans_y = 0.0
    o_img = np.copy(img)
    y = None
...
def get_temporary_corners(img, sess, x, output):
        o_img = np.copy(img)
        myImage = np.copy(o_img)
        img_temp = cv2.resize(myImage, (32, 32))
        img_temp = np.expand_dims(img_temp, axis=0)
...
def find_corners_in_image(img):
    graph, x, y = load_graph("./TrainedModel/cornerRefiner.pb", "Corner/inputTensor", "Corner/outputTensor")
    graphCorners, xCorners, yCorners = load_graph("./TrainedModel/getCorners.pb", "Input/inputTensor", "FCLayers/outputTensor")
    sess = tf.Session(graph=graph)
    sessCorners = tf.Session(graph=graphCorners)
    data = get_temporary_corners(img, sessCorners, xCorners, yCorners)
    corners_as_tuples = []
    for b in data:
        a = b[0]
        temp = np.array(refine_corner(a, sess, x, y, float("0.85")))
        temp[0] += b[1]
        temp[1] += b[2]
        corners_as_tuples.append(tuple(temp))
    return corners_as_tuples
